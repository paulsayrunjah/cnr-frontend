...
export default function LeadCard({ lead }: LeadCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-shadow p-6 border border-gray-100 dark:border-gray-700">
      ...
      {lead.business_types && lead.business_types.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {lead.business_types.slice(0, 3).map((type, index) => (
            <span
              key={index}
          ...
          {lead.business_types.length > 3 && (
            <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs rounded-md">
              +{lead.business_types.length - 3} more
            </span>
          )}
        </div>
      )}
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="text-xs text-gray-500 dark:text-gray-400">
          Discovered: {formatDate(lead.first_discovered)}
        </div>
        <a
